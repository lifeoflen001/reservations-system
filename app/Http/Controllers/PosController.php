...
class PosController extends Controller
{
    ...
    public function orders(Request $request, CurrencyFormatter $formatter)
    {
        ...
        return view('pos.orders', ['orders' => $orders, 'outlets' => PosOutlet::where('is_active', true)->orderBy('name')->get(), 'formatter' => $formatter, 'kpis' => [['label' => 'Today sales', 'value' => $formatter->format((clone $base)->sum('total')), 'icon' => 'currency', 'tone' => 'warning'], ['label' => 'Orders today', 'value' => (string) (clone $base)->count(), 'icon' => 'document', 'tone' => 'info'], ['label' => 'Room charges', 'value' => $formatter->format($roomCharges->sum('total')), 'icon' => 'bed', 'tone' => 'success']]]);
    }

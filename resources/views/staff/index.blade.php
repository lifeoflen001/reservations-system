...
@if(!request('tab') || request('tab') === 'staff')
    ...
    <form class="filter-toolbar staff-filters" method="GET" action="{{ route('staff.index') }}">
        ...
        <x-form.select name="activity" aria-label="Filter by recent activity"><option value="all">All activity</option><option value="recent" @selected(request('activity') === 'recent')>Recently active</option></x-form.select>

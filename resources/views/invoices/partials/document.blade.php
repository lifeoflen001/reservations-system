@php
    ...
    $refunds = $payment?->refunds?->where('status', 'posted') ?? collect();
    $netPayment = $amount - (float) $refunds->sum('amount');
...
<article class="invoice-document">
    ...
    <table class="invoice-lines"><thead><tr><th>Description</th><th>Quantity</th><th>Unit price</th><th>Total</th></tr></thead><tbody><tr><td><strong>Accommodation payment</strong><small>Reservation {{ $reservation?->code ?? '—' }} | Room {{ $reservation?->room?->room_number ?? '—' }} · {{ $reservation?->room?->roomType?->name ?? 'Room' }} | {{ $reservation?->check_in?->format('m/d/Y') ?? '—' }} - {{ $reservation?->check_out?->format('m/d/Y') ?? '—' }}</small></td><td>1</td><td>{{ $formatter->format($amount) }}</td><td>{{ $formatter->format($amount) }}</td></tr>@foreach($roomCharges as $charge)@php($description = $charge->order?->items?->pluck('product_name_snapshot')->filter()->join(', ') ?: 'POS room charge')<tr><td><strong>{{ $description }}</strong><small>POS order {{ $charge->order?->order_number ?? '—' }} · Outlet {{ $charge->order?->outlet?->name ?? '—' }} · Date {{ $charge->posted_at?->format('m/d/Y, h:i A') ?? '—' }} · POS room charge</small></td><td>1</td><td>{{ $formatter->format($charge->amount) }}</td><td>{{ $formatter->format($charge->amount) }}</td></tr>@endforeach @foreach($refunds as $refund)<tr><td><strong>Payment refund</strong><small>Refund {{ $refund->refund_reference }} · {{ $refund->method }} · {{ $refund->refunded_at?->format('m/d/Y, h:i A') }} · {{ $refund->reason }}</small></td><td>1</td><td>-{{ $formatter->format($refund->amount) }}</td><td>-{{ $formatter->format($refund->amount) }}</td></tr>@endforeach</tbody></table>
    <div class="invoice-total"><span>Net payment received <strong>{{ $formatter->format($netPayment) }}</strong></span><span>Total due <strong>{{ $formatter->format($totalDue) }}</strong></span><span>Balance <strong>{{ $formatter->format(max(0, $totalDue - app(\App\Services\FinancialService::class)->paidAmount($reservation))) }}</strong></span></div>
